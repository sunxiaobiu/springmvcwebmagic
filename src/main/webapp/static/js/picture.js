...
Picture.prototype.uploadProgress = function () {
    ...
    setTimeout(function () {
        _this.counter == _this.len && _this.$proContainer.addClass('Hide');
    }, 1000);
};
...
Picture.prototype.showPopbox = function () {
    ...
    //由于图片展示出来需要时间，所以等100ms在设置居中
    setTimeout(function(){
        var maskWidth = _this.$mask.width();
        var maskHeight = _this.$mask.height();
        if ((_this.$popImg.width() >= _this.$popImg.height()) && _this.$popImg.width()>maskWidth) {
            _this.$popImg.width(maskWidth/2);
        }

        if ((_this.$popImg.height() > _this.$popImg.width()) && _this.$popImg.height()>maskHeight) {
            _this.$popImg.height(maskHeight/2);
        }
        _this.setCenter(_this.$popImg);
    },100);
};

Picture.prototype.getMaxNumber = function(){
    return this.opt.maxNumber;
};

Picture.prototype.setMaxNumber = function(maxNumber){
...
Picture.prototype.bindEvent = function () {
    ...
    _this.$file.on('click', function () {
        if( _this.$imageList.find("li").size()<_this.getMaxNumber()){
    ...
    _this.$file.on('change', function () {
        _this.change();
        _this.$proContainer.length && _this.$proContainer.removeClass('Hide');

    });
    _this.$imageList.on("click", "li", function (e) {
    ...
    _this.$attachmentList.on("click", "li", function (e) {
        if(e.target.nodeName.toLowerCase()!='a'){
            var $li = $(this);
            _this.$CurLi = $li;
            _this.showPopbox();
    ...
    if (_this.edit) {
        _this.$popBox.on('click', '.remove', function () {
            ...
            _this.$file.val("");
            _this.$imageList.find("span").removeClass('hide');
            return false;
        });
    }
...
Picture.prototype.change = function () {
    ...
    if( _this.$imageList.find("li").size() + file.files.length > _this.getMaxNumber()){
        alert('上传图片张数超过上限');
        return false;
    }
    ...
    _this.$imageList.find("span").addClass('hide');
};
...
Picture.prototype.cache = function () {
    ...
    _this.$attachmentList = this.el.find("#attachmentList");
    _this.$proContainer = _this.progress?this.el.find('.progress'):[];
...
Picture.prototype.init = function () {
    ...
    _this.cache();
    _this.$proContainer.length && _this.$proContainer.addClass('Hide');
    _this.bindEvent();
};
